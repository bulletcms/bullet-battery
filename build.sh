...
# installs rkt
if ! [ -e "$PWD/build_modules/rkt" ]; then
  wget https://github.com/coreos/rkt/releases/download/v1.2.1/rkt-v1.2.1.tar.gz -O rkt.tar.gz && mkdir "$PWD/build_modules/rkt" && tar -xvzf rkt.tar.gz -C "$PWD/build_modules/rkt" --strip-components 1 && rm rkt.tar.gz
fi
